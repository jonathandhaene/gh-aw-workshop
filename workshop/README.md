...
- [Prevent Race Conditions with Concurrency Controls](side-quest-22-01-concurrency-controls.md) — add a `concurrency:` block to prevent duplicate writes when two runs trigger close together; covers group keys, `cancel-in-progress` strategies, and PR-scoped concurrency; branches from [Step 22](22-error-handling-and-resilience.md).
